    ...
    public XOGame(Socket socket, boolean isServer, String myName, String opponentName) {
        ...
        btnSurrender.addActionListener(e -> {
            if (getWinner() == ' ' && !isBoardFull()) {
                ...
                if (isServer) {
                    updateMatchHistory(myName, opponentName, "loss", 0, 1);
                }
                try {
                ...
                updatePlayersLabel();
                JOptionPane.showMessageDialog(this, "Bạn đã đầu hàng. " + opponentName + " thắng! Tỉ số: " + myName + ": " + myScore + " - " + opponentName + ": " + opponentScore);
                resetBoard();
            }
        });
        ...
        try {
            while (true) {
                ...
                if ("MOVE".equals(command)) {
                    ...
                    opponentName = command.substring(5);
                    setTitle("Game XO - " + myName + " vs " + opponentName);
                    updatePlayersLabel();
                } else if ("RESET".equals(command)) {
                    resetBoard();
                } else if (command.startsWith("SURRENDER:")) {
                    String surrenderName = command.substring(10);
                    if (surrenderName.equals(opponentName)) {
                        myScore += 1;
                        opponentScore += 0;
                        ...
                        updatePlayersLabel();
                        JOptionPane.showMessageDialog(this, opponentName + " đã đầu hàng. Bạn thắng! Tỉ số: " + myName + ": " + myScore + " - " + opponentName + ": " + opponentScore);
                        resetBoard();
                    }
                } else if ("EXIT".equals(command)) {
                    JOptionPane.showMessageDialog(this, opponentName + " đã thoát. Trò chơi kết thúc.");
                    closeGame();
                    SwingUtilities.invokeLater(() -> {
                        if (GameMenu.getActiveGameMenu() == null || !GameMenu.getActiveGameMenu().isVisible()) {
                            GameMenu newMenu = new GameMenu(myName);
                    ...
                    break;
                } else if ("RESET_REQUEST".equals(command)) {
                    int option = JOptionPane.showConfirmDialog(this, opponentName + " muốn chơi lại. Bạn có đồng ý không?", "Yêu cầu chơi lại", JOptionPane.YES_NO_OPTION);
                    if (option == JOptionPane.YES_OPTION) {
                        out.writeUTF("RESET_ACCEPT");
                        out.flush();
                        resetBoard();
                        JOptionPane.showMessageDialog(this, "Bạn đã chấp nhận chơi lại.");
                    } else {
                        out.writeUTF("RESET_DENY");
                        out.flush();
                    }
                } else if ("RESET_ACCEPT".equals(command)) {
                    resetBoard();
                    JOptionPane.showMessageDialog(this, opponentName + " đã chấp nhận chơi lại.");
                } else if ("RESET_DENY".equals(command)) {
                    JOptionPane.showMessageDialog(this, opponentName + " đã từ chối chơi lại.");
                }
            }
        } catch (IOException e) {
            if (!isExiting) {
